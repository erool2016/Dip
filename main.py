...
def presentation(list,user_id):
    ...
    while count < 3:
        ...
        write_msg(user_id,f'{list[count][1]},{list[count][2]},"лайков",{list[count][3]}',attachment=list[count][4])
        count += 1
...
def create_list_for_prezentation(list,user_id):
    ...
    for item in list:
        answer = Data_Base.send_db(item[0])
        if answer == None:
            list_for_prezenatation.append(item)
